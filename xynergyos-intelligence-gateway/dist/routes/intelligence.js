 ...
 */
router.get('/daily-briefing', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const userId = req.user.uid;
    const tenantId = req.tenantId || 'default';
    logger_1.logger.info('Fetching daily briefing', { userId, tenantId });
    try {
        ...
        logger_1.logger.error('Failed to fetch daily briefing', { error: error.message, userId });
        res.status(500).json({
            success: false,
 ...
 */
router.get('/content/suggestions', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const userId = req.user.uid;
    const limit = parseInt(req.query.limit) || 20;
    const minScore = parseFloat(req.query.min_opportunity_score) || 0;
    logger_1.logger.info('Fetching content suggestions', { userId, limit, minScore });
    try {
        ...
        logger_1.logger.error('Failed to fetch content suggestions', { error: error.message, userId });
        res.status(500).json({
            success: false,
 ...
 */
router.post('/content/generate-brief', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const userId = req.user.uid;
    const { suggestion_id, keywords, topic } = req.body;
    logger_1.logger.info('Generating content brief', { userId, suggestionId: suggestion_id });
    try {
        ...
        logger_1.logger.error('Failed to generate content brief', { error: error.message, userId });
        res.status(500).json({
            success: false,
 ...
 */
router.get('/opportunities', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const userId = req.user.uid;
    const category = req.query.category;
    const status = req.query.status;
    const limit = parseInt(req.query.limit) || 50;
    logger_1.logger.info('Fetching opportunities', { userId, category, status, limit });
    try {
        ...
        logger_1.logger.error('Failed to fetch opportunities', { error: error.message, userId });
        res.status(500).json({
            success: false,
 ...
 */
router.get('/competitive', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const userId = req.user.uid;
    const competitor = req.query.competitor;
    const metric = req.query.metric;
    logger_1.logger.info('Fetching competitive intelligence', { userId, competitor, metric });
    try {
        ...
        logger_1.logger.error('Failed to fetch competitive intelligence', { error: error.message, userId });
        res.status(500).json({
            success: false,
 ...
 */
router.get('/predictions', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const userId = req.user.uid;
    const category = req.query.category;
    const timeframe = req.query.timeframe || '30d';
    logger_1.logger.info('Fetching predictions', { userId, category, timeframe });
    try {
        ...
        logger_1.logger.error('Failed to fetch predictions', { error: error.message, userId });
        res.status(500).json({
            success: false,
...
router.get('/notifications', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    logger_1.logger.info('Fetching notifications', { userId, unreadOnly });
    try {
        ...
        logger_1.logger.error('Failed to fetch notifications', { error: error.message, userId });
...
router.post('/notifications/:id/read', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const userId = req.user.uid;
    const { id } = req.params;
    logger_1.logger.info('Marking notification as read', { userId, notificationId: id });
    try {
        ...
        logger_1.logger.error('Failed to mark notification as read', { error: error.message, userId, id });
        res.status(500).json({
            success: false,
 ...
 */
router.post('/notifications/mark-all-read', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const userId = req.user.uid;
    logger_1.logger.info('Marking all notifications as read', { userId });
    try {
        const notificationsRef = firestore
            .collection('notifications')
            .where('user_id', '==', userId)
        ...
        logger_1.logger.error('Failed to mark all notifications as read', { error: error.message, userId });
        res.status(500).json({
            success: false,
...
router.delete('/notifications/:id', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const userId = req.user.uid;
    const { id } = req.params;
    logger_1.logger.info('Deleting notification', { userId, notificationId: id });
    try {
        const notificationRef = firestore.collection('notifications').doc(id);
        const doc = await notificationRef.get();
        if (!doc.exists) {
        ...
        logger_1.logger.error('Failed to delete notification', { error: error.message, userId, id });
        res.status(500).json({
            success: false,
