...
router.get('/available', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    logger_1.logger.info('Fetching available integrations', {
        userId: req.user?.uid,
    });
    res.json({
        success: true,
...
router.get('/list', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    logger_1.logger.info('Fetching user integrations', { userId });
    try {
        ...
        logger_1.logger.error('Failed to list integrations', { error: error.message, userId });
        res.status(500).json({
            success: false,
 ...
 */
router.post('/connect', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    logger_1.logger.info('Initiating OAuth flow', { userId, provider });
    ...
    switch (provider) {
        ...
        case 'calendar': {
            ...
            const callbackUri = redirect_uri || `${process.env.API_BASE_URL}/api/v1/integrations/callback`;
            const scopes = providerConfig.scopes.map((s) => s.includes('://') ? s : `https://www.googleapis.com/auth/${s}`).join(' ');
            authUrl =
...
router.get('/callback', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    const provider = queryProvider || 'slack'; // Default to slack for backward compatibility
    if (oauthError) {
        logger_1.logger.error('OAuth callback error', { error: oauthError, provider });
    ...
    logger_1.logger.info('OAuth callback received, forwarding', {
        provider,
        hasCode: !!code,
        hasState: !!state,
    });
...
router.delete('/:id', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    logger_1.logger.info('Disconnecting integration', {
        userId,
        service,
        integrationId: id,
    ...
    try {
        ...
        logger_1.logger.info('Integration disconnected', { userId, service });
        res.json({
            success: true,
        ...
        logger_1.logger.error('Failed to disconnect integration', {
            error: error.message,
            userId,
            service,
        });
 ...
 * Response: { success: boolean }
 */
router.post('/sync/:provider', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    const hasToken = await tokenManager_1.tokenManager.hasValidToken(userId, provider);
    if (!hasToken) {
    ...
    logger_1.logger.info('Sync triggered', { userId, provider });
    res.json({
        success: true,
 ...
 */
router.get('/:id/status', (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { id } = req.params;
    const userId = req.user.uid;
    const service = id.split('_').pop();
    if (!['slack', 'gmail', 'calendar'].includes(service)) {
    ...
    logger_1.logger.info('Checking integration status', { userId, service });
    try {
        ...
        logger_1.logger.error('Failed to check integration status', {
            error: error.message,
            userId,
            service,
        });
